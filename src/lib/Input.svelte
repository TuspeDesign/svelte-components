<script lang="ts">
  ...
  let classesIn = $derived(inputClass),
...
<style scoped>
  ...
  input,
  textarea {
    background-color: var(--color-input-bg, transparent);
    border-radius: var(--rounded-input, 0.375rem);
    border: 1px solid var(--color-border, #bbb);
    color: var(--color-input, var(--color-content, #222));
    display: block;
    font-size: 1rem;
    margin-top: 0.25rem;
    padding-left: 0.5rem;
  ...
  label.filter {
    font-size: var(--text-label-size, var(--text-sm, 0.95rem));
  }
